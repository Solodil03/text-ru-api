...
TEXT_RU_API_KEY = "3c957569f16140b682c791af6ec3176d"  # Твой API-ключ
TEXT_RU_API_URL = "https://api.text.ru/post"
TEXT_RU_GET_URL = "https://api.text.ru/post"
...
@app.get("/get_result")
def get_result(uid: str):  # Параметр теперь uid
    payload = {
        "uid": uid,
        "userkey": TEXT_RU_API_KEY,
        "jsonvisible": "detail"
    ...
    response = requests.post(TEXT_RU_GET_URL, data=payload)  # Используем POST
    logging.info(f"Ответ от Text.ru: {response.text}")
